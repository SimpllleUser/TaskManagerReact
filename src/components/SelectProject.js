...
import { Heading, PlusIcon, Button, Dialog } from "evergreen-ui";
...
const SelectProject = () => {
  ...
  return (
    <div className="selector_project nav-item dropdown">
      {projects.length <= 1 ? (
        <a className="navbar-brand h4 text-white border rounded">
          {projects[0]?.title}
        </a>
      ) : (
        <div className="nav-item dropdown">
          <a
            className="h4 text-white nav-link dropdown-toggle border rounded"
            href="#"
            id="dropdownProject"
            role="button"
